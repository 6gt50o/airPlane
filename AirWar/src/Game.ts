...
class Game {
    ...
    onLoop(): void {
        ...
        for (var i: number = this.roleBox.numChildren - 1; i > -1; i--) {
            ...
            if (role && role.speed) {
                ...
                //如果敌人移动到显示区域以外，则移除
                if (role.y > 1000 || !role.visible || (role.heroType === 1 && (role.y < -20 || role.y > 872))) {
                    //从舞台移除
                    role.removeSelf();
                    //回收之前，重置属性信息
                    role.visible = true;
            ...
            if (role.shootType > 0) {
                ...
                if (time > role.shootTime) {
                    ...
                    //     //初始化子弹信息，根据不同子弹类型，设置不同的飞行速度
                    //     if(role.type === "enemy2"){
                    //           bullet.init("bullet1", role.camp, 1, 4 +　 Math.floor(this.level / 15)　, 1, 1);
                    //     }
                    //     else if(role.type === "enemy3"){
                    //          bullet.init("bullet1", role.camp, 1,5, 1, 1); 
                    //     }             
                    //     //设置子弹发射初始化位置
        ...
        for (var i: number = this.roleBox.numChildren - 1; i >= 0; i--) {
            ...
            for (var j: number = i - 1; j >= 0; j--) {
                ...
                //如果角色未死亡，并且阵营不同，才进行碰撞,而且子弹之间不进行碰撞
    ...
    createEnemy(type: number, num: number, speed: number, hp: number): void {
        for (var i: number = 0; i < num; i++) {
            ...
            // if(type === 1){
            //     enemy.shootType = 1;
            //     enemy.shootInterval = 4000;
            // }
            // else if(type === 2){
            //     enemy.shootType == 1;
            //     enemy.shootInterval = 8000;
            // }      
               //随机位置
